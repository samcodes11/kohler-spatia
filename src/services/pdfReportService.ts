...
doc.text('© 2026 Spatia. Concept case study — not affiliated with any specific fixture brand.', margin, pageHeight - 9);
...
doc.setFontSize(7.5);
doc.setTextColor(stoneGrey[0], stoneGrey[1], stoneGrey[2]);
doc.text('© 2026 Spatia. Concept case study — not affiliated with any specific fixture brand.', margin + 6, pageHeight - 15);
...
{
cat: 'SHOWER SYSTEM',
chosen: 'Thermostatic Rain & Jet Column',
...
chosen: 'Freestanding / Floating Console',
...
chosen: 'Smart Tankless / Wall-Hung System',
...
{
cat: 'BASIN FAUCET',
chosen: 'Precision Faucet with Laminar Aerator',
...
doc.setFontSize(7.5);
doc.setTextColor(stoneGrey[0], stoneGrey[1], stoneGrey[2]);
doc.text('Concept case study — not affiliated with any specific fixture brand.', margin, y);
doc.text('All product names, trademarks and registered trademarks are property of their respective owners.', margin, y + 5);
doc.text('Generated deterministically by Spatia Spatial Intelligence Engine.', margin, y + 10);

// Save the PDF
const filename = `Spatia_Design_Report_${Date.now().toString().slice(-6)}.pdf`;
doc.save(filename);
return doc;
}
